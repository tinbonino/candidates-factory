...
REQUIRED JSON SCHEMA:
{
  ...
  "skills": ["string — list of 8 to 12 specific, named technical and professional skills. Prefer precise tool and platform names extracted verbatim from the CV (e.g. 'Salesforce Inspector', 'Data Loader', 'Flow Builder', 'Validation Rules', 'Custom Objects') over generic category labels (e.g. 'Data Loads', 'Salesforce Configuration'). Include both technical tools and cross-functional skills."],
...
- TITLE: If the job description specifies a seniority level (e.g. Senior, Lead, Principal, Staff) and the candidate's work history clearly supports it, reflect that seniority in the `title` field (e.g. 'Salesforce Business Analyst' → 'Senior Salesforce Business Analyst'). Do not elevate if the CV does not support it.
- SUMMARY: Rewrite to foreground the candidate's REAL experience that best matches the job description. Lead with concrete scale and scope differentiators — e.g. number of users supported, platform types, geographic scope, delivery methodology, language of operation. These specific facts make the candidate stand out. Do NOT state a specific total number of years of experience (avoid 'X years of experience', 'over N years'). Describe seniority qualitatively.
- SKILLS: Expand the skills list (up to 20 items) by surfacing specific tool and platform names mentioned anywhere in the CV — including inside role descriptions — that align with the JD keywords. Use precise names (e.g. 'Data Loader', 'Salesforce Inspector', 'Flow Builder', 'Validation Rules') not generic labels. Order skills so the most JD-relevant appear first. Do NOT add any skill not evidenced in the CV.
- EXPERIENCE: Keep the `experience` array STRUCTURALLY IDENTICAL to the CV: the same number of roles, the same companies, the same role titles, the same date periods. Do NOT split a single role into multiple entries, do NOT merge roles, do NOT add or remove any role. Only reorder the `achievements` array within each role so the most job-relevant ones come first. Keep EVERY achievement.
- PROFILE: Rewrite `core_expertise`, `technical_highlights`, `key_strengths`, `integration_skills`, `professional_badges`, and `qualitative_profile` to emphasize the competencies the job description requires, using only evidence already present in the CV.
